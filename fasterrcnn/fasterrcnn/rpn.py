...
import torchvision
from fasterrcnn import boxes as box_ops

from fasterrcnn  import _utils as det_utils
from fasterrcnn.image_list import ImageList
...
class AnchorGenerator(nn.Module):
    ...
    def forward(self, image_list, feature_maps):
        ...
        # [torch.Size([304, 304]), torch.Size([152, 152]), torch.Size([76, 76]), torch.Size([38, 38]), torch.Size([19, 19])]
        grid_sizes = list([feature_map.shape[-2:] for feature_map in feature_maps])
        # torch.Size([1216, 1216])
        image_size = image_list.tensors.shape[-2:]

        dtype, device = feature_maps[0].dtype, feature_maps[0].device

        #strides:  [[tensor(4), tensor(4)], [tensor(8), tensor(8)], [tensor(16), tensor(16)], [tensor(32), tensor(32)], [tensor(64), tensor(64)]]
        strides = [[torch.tensor(image_size[0] / g[0], dtype=torch.int64, device=device),
                    torch.tensor(image_size[1] / g[1], dtype=torch.int64, device=device)] for g in grid_sizes]
                  
        self.set_cell_anchors(dtype, device)
        ## anchors_over_all: 
        ## type: List[Tensor]  shape for every one: [feature_widht*feature_height, 4]
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        # anchors : 每张图片上的所有 anchor [anchor_size, 4]
        # targets: [{"boxes", "label":}]
        labels = []
        matched_gt_boxes = []
        # 对每个样本迭代
    ...
    def forward(self, images, features, targets=None):
        ...
        # type: list[Tensor], len(ancors)  = batch_size
        ...
        # the proposals
        # num_image * num_anchor_per_image * 4
        ...
        if self.training:
            ...
            # len(anchors) = num_iamges, [anchor_size, 4]
